...
void Square_Key_Take(CommandContext context, GameState* gameState, WorldData* worldData)
{
...
void Square_Key_Use(CommandContext context, GameState* gameState, WorldData* worldData)
{
	...
	if (GameFlags_IsInList(gameState->gameFlags, "squareKeyUsed"))
	{
	...
	else
	{
		...
		key = ItemList_FindItem(gameState->inventory, "SquareKey");
		...
		gameState->gameFlags = GameFlags_Add(gameState->gameFlags, "squareKeyUsed");
...
/* Build a "key" object */
Item* Square_Key_Build()
{
	/* Create a "key" item, using the functions defined in this file */
	return Item_Create("square key", "A small smooth gem, completely transparent", true, Square_Key_Use, Square_Key_Take, NULL);
}